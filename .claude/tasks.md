...
- [x] **4c-9** (BL-LD1): Go loader sanitize — phase-3 file-based loader が持っていた behavior enum + items 一貫性 + name char allow-list の検証を、phase-4a の AWS XML handler 経由 path にも適用。`internal/api/validate/policy.go` 新規 (CachePolicyConfig / OriginRequestPolicyConfig / ResponseHeadersPolicyConfig)、3 ハンドラの `decodeConfig` で wrapper.ToSDK() 直後に `validate.*Config(cfg)` を呼び失敗時は `awsxml.WriteError(CodeInvalidArgument)`。allow-list は phase-3 loader と統一 (`A-Za-z0-9._-`)、ResponseHeadersPolicy の Custom/Remove header 名は HTTP token char (RFC 7230) を許容。policy_test.go 36 ケース (cache policy 全 enum + items 違反 + Header 名 CRLF/NUL/制御文字 / ORP allViewer 系 / RHP token validation) PASS
...
- [x] **4c-11**: 積みタスク BL-W1/W2/IV1/IV2/PP1/NX3 (+ BL-IM1/NX1/NX2/LD1/OB1/RV1/RV2) を `docs/limitations.md` の「積みタスク (backlog) 一覧」表として 1 段落に集約。完了済 (BL-NX1/LD1/OB1) は **完了** マークで識別、未着手は条件付きで残す
