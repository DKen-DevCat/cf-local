...
- [x] **4b-8**: ListInvalidations handler + Marker pagination (2026-05-02) — `internal/api/invalidation/aws_handler.go` の `AWSHandler.List` + `parseListMaxItems` + `paginateRecords`、9 ケース (sub-test 含 11 件) 全 PASS。GET `/2020-05-31/distribution/{distId}/invalidation`、`MaxItems` (default 100 / >100 を 100 に clamp / 非数 ・ ≤0 は 400 InvalidArgument) と `Marker` (前ページ末尾の ID と一致したら次の record から開始 / unknown marker は AWS-permissive に空ページ返却) を query で受け、`Store.List` の CreateTime DESC + ID tie-break をそのまま slice。`IsTruncated` が true のときのみ `NextMarker` を最後の ID で出す (omitempty)。`Quantity` は Items 長さから自動算出
- [x] **4b-9**: phase-3 独自 `POST /_invalidate` 撤去 (2026-05-02) — handler.go / purger.go / handler_test.go / purger_test.go / 旧 α 統合テスト + server.go の `/_invalidate` route + `Config.NginxURL` + main.go の `--nginx-url` flag + nginx config の `/_cf_purge<path>` location + 3 つの golden fixture を削除。AWS 互換 worker (4b-6 / B-2 cache walk) が機能的上位互換のため互換層は残さず一本化。docker-compose.yml に cf-local 側の `nginx-cache:/var/cache/nginx` mount + `--cache-dir` flag を追加 (4b-6 worker の実機動作に必要)。go test ./internal/... ./cmd/... 全 PASS、nginx golden 更新済
...
- [x] **4b-10**: AWS XML α 統合テスト追加 + examples 拡充 (2026-05-02) — `tests/integration/invalidation_aws_alpha_test.go` を新規作成 (旧 simple-JSON 版を AWS REST/XML 経由に置換)。CreateInvalidation → GetInvalidation polling で Completed まで wait → cache MISS 検証、wildcard `/...*` の multi-slot purge、multi paths 1 リクエスト、未 cache パスの空 walk、invalid path 400 InvalidArgument、ListInvalidations smoke の 6 sub-test。skip mode で build / vet / `-run` 経路 PASS 確認済 (実機 docker-compose 検証はユーザー実行に委ねる)。`examples/terraform-integration/README.md` に AWS CLI で create-invalidation / get-invalidation / list-invalidations を叩く手順 + 想定外 API 呼び出し log 確認手順を追記
