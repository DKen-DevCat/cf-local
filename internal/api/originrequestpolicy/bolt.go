...
// BoltBucket is the bbolt bucket name for persisted origin request
// policies.
var BoltBucket = []byte("origin_request_policies")

type boltRecord struct {
	ID               string                           `json:"id"`
	ETag             string                           `json:"etag"`
	Config           *types.OriginRequestPolicyConfig `json:"config"`
	LastModifiedTime time.Time                        `json:"last_modified_time"`
}

// BoltStore persists origin request policies to a bbolt bucket while
// serving Get/List from an in-memory cache. Mirrors cachepolicy.BoltStore
// minus managed-policy support (ORP managed policies are out of scope
// for phase-4a).
...
// Create inserts a new origin request policy.
func (s *BoltStore) Create(_ context.Context, cfg *types.OriginRequestPolicyConfig) (*Record, error) {
...
func (s *BoltStore) Update(_ context.Context, id string, cfg *types.OriginRequestPolicyConfig, ifMatch string) (*Record, error) {
	if cfg == nil || cfg.Name == nil {
		return nil, errors.New("origin request policy: missing name")
	}
	_ = ifMatch
