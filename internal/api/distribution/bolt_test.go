...
func TestBoltStore_PersistAcrossReopen(t *testing.T) {
	...
	rec, err := s1.Create(context.Background(), newDistConfig("persisted"))
	if err != nil {
	...
	if aws.ToString(got.Config.CallerReference) != "persisted" {
		t.Errorf("CallerReference lost on reload: got %q",
			aws.ToString(got.Config.CallerReference))
	}
}
...
func TestBoltStore_UpdateAndDeletePersist(t *testing.T) {
	...
	rec, err := s.Create(ctx, newDistConfig("upd-test"))
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	updated := newDistConfig("upd-test")
...
func TestBoltStore_DuplicateCallerReferenceRejected(t *testing.T) {
	db := openTestDB(t)
	s, err := NewBoltStore(db)
	if err != nil {
		t.Fatal(err)
	...
	ctx := context.Background()
	if _, err := s.Create(ctx, newDistConfig("dup-bolt")); err != nil {
		t.Fatalf("first Create: %v", err)
	}
	if _, err := s.Create(ctx, newDistConfig("dup-bolt")); !errors.Is(err, ErrAlreadyExists) {
