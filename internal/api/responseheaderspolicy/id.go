...
// AWS CloudFront response-headers-policy IDs use the same shape as cache
...
func newResponseHeadersPolicyID() (string, error) {
	buf := make([]byte, 9)
...
// responseHeadersPolicyETag returns a stable opaque identifier for the
...
func responseHeadersPolicyETag(cfg *types.ResponseHeadersPolicyConfig) string {
	if cfg == nil {
		return ""
	}
