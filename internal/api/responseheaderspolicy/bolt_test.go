...
func TestBoltStore_OnChangeFires(t *testing.T) {
	db := openTestDB(t)
	s, err := NewBoltStore(db)
	if err != nil {
		t.Fatal(err)
	...
	ctx := context.Background()
	rec, err := s.Create(ctx, newPolicyConfig("oc"))
	if err != nil {
		t.Fatal(err)
	}
	if calls != 1 {
		t.Errorf("after Create: calls=%d want 1", calls)
	}
	if _, err := s.Update(ctx, rec.ID, newPolicyConfig("oc"), ""); err != nil {
		t.Fatal(err)
	}
	if calls != 2 {
		t.Errorf("after Update: calls=%d want 2", calls)
	}
