...
func writeServerBlock(b *bytes.Buffer, behaviors []behaviorView, inners []innerView) {
    ...
    #
    # アクセス制御: loopback + RFC1918 private range を許可。docker compose
    # network (default 172.x) と Control Plane container からの purge を受ける
    # ため。host からの直接叩きは 8080 を通るが、その場合も source は 127.0.0.1
    # に見えるので allow される (security 観点では緩いが Phase 3 はローカル
    # 開発ツール前提)。Phase 4-A の REV-1 で unix socket 化して根本解決予定。
    location ~ ^/_cf_purge(/.*)$ {
        allow 127.0.0.1;
        allow 10.0.0.0/8;
        allow 172.16.0.0/12;
